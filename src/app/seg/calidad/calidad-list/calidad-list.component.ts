...
import {CalidadService} from 'src/app/seg/service/calidad.service'
...
@Component({
  ...
  templateUrl: './calidad-list.component.html',
  styleUrls: ['./calidad-list.component.scss']
})
export class CalidadListComponent implements OnInit {
  ...
  ['id', 'proyecto', 'tipo_de_celda', 'cell_name', 'banda', 'nombre_de_sitio',
  'frb', 'cqi_af','th_af','rru_neiran','pa_neiran','rs_neiran','set_number',
  'eval_control','result','created_at'];
  ...
  ngOnInit(): void {
    this.dataSource = new PaginatedDataSource<any>(
      (request: PageRequest<any>) => this.calidadService.getAll(request),
  ...
  export(): void {
    ...
    .subscribe((data) => Util.download(data, 'export-monitoreo_flujo_calidad'));
  }
